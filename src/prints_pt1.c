...
/*   Updated: 2019/09/24 16:02:13 by awali-al         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	dis_perm(mode_t mode)
{
	...
	if ((mode & S_ISVTX) != S_ISVTX)
		...
		(mode & S_IXOTH) ? write(1, "t", 1) : write(1, "T", 1);
	write(1, " ", 1);
}
...
void	dis_user(uid_t uid, int max)
{
	...
	d = max - ft_strlen(getpwuid(uid)->pw_name) + 2;
	i = 0;
...
void	dis_grup(gid_t gid, int max)
{
	...
	d = max - ft_strlen(getgrgid(gid)->gr_name) + 1;
	i = 0;
